...
- **On the p01 pilot**, the Codex paper that introduced HumanEval comes first in both runs: cited by 6 of 7 seeds in the evidence-version-3 bibliography and 5 of 8 in the version-4 one, and never by the study. MBPP, SWE-bench, and SWE-agent follow; 29 of the top 30 are works the study had not cited.
...
`make` wraps the common ones: `setup`, `test`, `lint`, `diagnose`, `graph`, `postgres-up`, `postgres-down`, `db-status`, `migrate`. The diagrams in this README are generated by `scripts/readme_diagrams.py`.
