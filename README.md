...
None of these commands cost anything. The `synthetic` policy runs the real graph and repository with scripted role outputs, which checks the wiring without provider calls. Postgres, `OPENROUTER_API_KEY`, and the first paid run are covered in [docs/setup.md](docs/setup.md).
