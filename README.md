...
research rescout <run id> --model zai:glm-5.3-flash --max-usd 0.50 --study scouts --arm flash --replicate 1
```

The `scout`, `synthesize`, `rescout`, `grade`, and `assess` commands call paid models. Scout prints its models and limits before it starts. It prints the report when it finishes, and with `--out` it also writes `report.md` and `run.json`. When `DATABASE_URL` is set the run is stored, including every model call's messages. `--study NAME --arm ARM --replicate N` labels a run as part of a study, so runs can be paired later. The dollar amounts in these examples are ceilings, not recommended spending or approval for a study.
...
`grade`, `assess`, `synthesize`, and `rescout` require `--max-usd`. Frozen-case Scout runs require it too; ordinary Scout runs may use it. The guarded paths reserve a conservative upper charge before every request and refuse a request that would exceed the command cap. Reservation policy `usage-anchor-v4` bounds a request's input from the last reply in its history: that reply's billed input and output tokens, plus one token per byte of everything added since, plus 4,000 tokens for framing. A first request, with no reply to start from, is bounded by twice its serialized bytes plus 16,000 tokens. The output bound is the configured output cap. When a response returns with usage that can be priced, its reservation is replaced by the actual charge. Version 3 bounded every request by its bytes, which reserved about eleven times a scout request's input and refused Luna scouts at a $0.50 cap; version 2 kept every reservation, so parallel scouts held most of the cap and synthesis was refused; version 1's four-times-byte estimate falsely refused long scout histories. This includes validation retries and the scouts' timed 429 retries. SDK retries and fallback are disabled; a request that fails, or whose usage cannot be priced, keeps its full reservation, except that a 429 rate-limit rejection releases it because the provider did not process the request. Guarded Scout requests cap planner output at 16,000 tokens, scout output at `RESEARCH_LIMITS__GUARDED_SCOUT_MAX_OUTPUT_TOKENS` (24,000 by default), and synthesis output at `RESEARCH_LIMITS__SYNTHESIS_MAX_OUTPUT_TOKENS` (32,000 by default). The grade judge caps output at 16,000 tokens. Guarded runs record the cap and policy in run configuration and final checks. Run `research db migrate` to add the assessment and grade budget columns before using these commands.
...
`research rescout` is the same idea for the scouts. It takes an existing run's plan, researches it again with the scout model given by `--model`, and stops before synthesis. It uses the production scout prompt, tools, evidence checks, dollar shares, and 4.5-minute research window. The new child run records the source run ID and a digest of the plan; it is complete when every question returned evidence. Because a Scout run's research window also covers planning, a rescout gets slightly more time than its source did, so compare rescouts with each other. `research synthesize` can then write a report from any rescout ledger. Both commands carry a frozen case's identity to the new run, so `research grade` accepts the report.
...
Both time limits count from the start of the run. Planning counts against the research phase and gets at most 90 seconds. The synthesizer gets whatever time is left once research ends, so when a scout runs to the research deadline it has at most 1.5 minutes. Each model request times out after 120 seconds (`RESEARCH_LIMITS__REQUEST_TIMEOUT_SECONDS`). A scout does not retry a request that hits that timeout. The provider client would otherwise send it again twice, and one slow first reply would fill the research window. When a provider returns a token rate limit with an explicit retry time, the scouts share that pause and retry the same request up to twice. An exhausted balance or a 429 without a retry time still fails the call. Retries alone do not hold under a tight limit, so scouts on a model listed in `RESEARCH_TOKENS_PER_MINUTE` are also paced. Before each request, the run waits until the tokens its scouts sent that model in the last minute, plus the request's estimate, fit under 90% of the limit. The estimate starts from the previous reply's billed tokens and counts four bytes a token for what was added since; billed tokens replace it when the reply returns. The default is `{"openai:gpt-6-luna": 200000}`, this account's OpenAI tier, which four unpaced Luna scouts overran; set `'{}'` to turn pacing off. Pacing is per run, so two runs at once can still reach the limit, and time spent waiting counts against the research window. Scout enables Python fault tracing, so a native parser crash prints its Python call stack to stderr.

The money is divided before the run starts. The planner gets $0.05, the synthesizer $0.40, and the scouts split the rest. In follow-up mode, $0.10 goes to gap analysis and $0.25 is reserved for one deep dive; the first scouts split the remaining $0.45. The deep dive is skipped when gap analysis finds no material gap. Follow-up reserves at least 90 seconds for synthesis. Each call stops before a request that would pass its share, so the total can exceed the cap by at most one request per call; these are soft limits, including in follow-up mode. Each scout is told on every request how much of its budget is left, and loses its tools on its last request, so it returns what it found instead of being cut off. A turn that asks for more tool calls than the scout's limit leaves runs only the calls that fit, and the next request says how many were dropped. It also loses them when less than one request timeout remains before the research deadline, so that last request can write the claims it has before the deadline discards them.
