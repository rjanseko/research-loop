...
`README.md` is the entry point and covers setup, configuration, and the Scout workflow; update it with any change to commands, settings, limits, or outputs, and keep its Mermaid diagram in step with `scout.py`. `docs/lessons.md` records what the first design learned; add to it rather than rewriting it. Rules above that name the graph, the legacy loop, or `research-graph-v1` are being rewritten for the Scout design.
