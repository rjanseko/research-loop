...
Finding 2 is partly addressed: the synthesizer's report and the verifier's checks may cite only claim IDs in the run's ledger, and an unknown ID gets one retry, then fails the run (`evidence_version` 3). The archived pilot shows why: it ran before claim IDs were unique, and all 38 of its report citations used a `q1:c1` form that matched no ledger ID. Still open: duplicate planner IDs, worker question-ID mismatches, verifier follow-ups for questions outside the plan, attachment and source observation checks, and a quality disposition separate from execution status.
