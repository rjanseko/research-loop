...
- *Found.* `scripts/resynthesize.py` on pilot 8's ledger, with synthesis streamed, received no bytes from Z.ai for 7.8 minutes before it was stopped; the trial job is `087b942e`, recorded failed (`CancelledError`). A small streamed call at `max` did stream its thinking, but only after 11.8 seconds, so Z.ai appears to reason before it sends anything, and on a 54,000-character ledger that runs past the 600-second read timeout. Streaming therefore does not save a `max` synthesis of this size; the fallback to `gpt-6-sol` would have written the report after about 30 minutes.
- *Changed.* The study synthesizes on `glm-5.3` at `high`, as pilot 4 did on this case in 180 to 256 seconds, keeping the 64,000-token output allowance. The model change's reason for `max`, fewer validation retries than `high` needed, is untested.
