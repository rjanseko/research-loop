...
The synthesizer then writes a `FinalReport` from a compact view of the ledger. Every claim in the report has to cite ledger claim IDs such as `q2/c1`, and the report text cites its sources inline as `[s3]`; each such source must be behind one of the claims the report lists, or the synthesizer gets one retry. Source IDs are numbered once over the whole ledger, so the synthesizer, the verifier, and `outcome.sources` all use the same ID for the same source. The verifier checks each of those claims against the evidence it cites, grades any problems by severity, and may ask for more research. If it does and rounds remain (`max_verification_rounds`), the follow-ups become deep dives, possibly on the policy's alternate deep-dive model, and synthesis and verification run again. Otherwise the run ends with its report, its verification, and its `review_reasons`. Those name a report whose statements the verifier checked only in part. Checks carry no statement IDs and verifiers merge and split statements, so a statement counts as checked when a check has the same words or the checks together cite all of its claims.
...
With `salvage_exhausted_research` turned on, a scout or deep dive that hits a usage limit, or gives up after repeated tool or output errors (`UnexpectedModelBehavior`, for example a fetch tool that kept failing), gets one more call without tools, which summarizes the evidence that call had already gathered. It sees only that call's tool results, not the whole ledger. Unanswered, failed, and repeated tool calls are left out, and the remaining results share a 64,000-character allowance so that later, more targeted fetches aren't crowded out. Quote checks still run against the captured tool text, but the stored salvage prompt keeps only hashes and sizes of the replayed results. The salvage call has 140,000 tokens, room for one validation retry with a long answer. If it fails as well, the question's result has no claims but keeps the loop's search queries and names the pages it read as follow-ups, so gap analysis can send a deep dive to them. A provider error that the model client's own retries did not clear (`ModelAPIError`, such as a 429 or a 5xx) gets no salvage call, which would go to the same provider: the result keeps the loop's queries and pages in the same way, and the job continues with a review reason naming the error. The planner, gap analyst, synthesizer, and verifier still fail the job on a provider error, so an exhausted balance stops it at the next of them, unless the route names a `refusal_fallback`, which the call then runs on (see below).
...
A route with `refusal_fallback` runs a call once more on that model when its own model refuses it (`ContentFilterError`), as Opus 5.5 refused to plan a DRB-II question on T-cell exhaustion. A retry on the same model would be refused again. Since 25 September 2026 it does the same when the provider fails (`ModelAPIError`) after the SDK's own retries: a timeout, a server error, or an exhausted balance. The failed call stays recorded as a failed task, and the fallback call is a task of its own on the fallback model; after a provider error the job also gets a note naming both models. The `quality`-based presets set it on the planner and synthesizer, to the preset's gap-analysis model.

The synthesizer's calls are streamed. PydanticAI's HTTP client times a read out after 600 seconds, and the OpenAI SDK, which several providers use, tries a request three times. A synthesis at high reasoning effort can write for longer than that before its reply begins, and unstreamed it then fails after 30 minutes; streamed, the timeout applies between chunks. Other roles are not streamed.
...
| `budget_notes` | none (off) | Experimental. The named tool-loop roles (`scout`, `deep_dive`) end each model request with a note of the requests and tool calls they have left, which invites parallel tool calls and asks for the result before a limit cuts the loop off. Earlier notes are resent unchanged, so the cached prompt prefix survives. The loop is also offered no tools on its last request, or once it has used its tool calls, so it returns its result instead of reaching a limit and needing a salvage call; its tool-call limit allows one more parallel batch of up to 12 calls (`TOOL_BATCH_SLACK`) to finish. It changes what models are sent, so compare quality before turning it on |
...
When an agent cites something, it records a `SourceRef` with those identifiers, a `locator` such as a page or section, the publication status, a short `excerpt`, and optionally a verbatim `quote`. A `source_type` or publication status outside its list, such as a source typed `dataset`, is stored as `unknown` rather than rejected, since a rejection would redo the whole call. Code then compares the citation with what the tools actually returned during that research call. `quote_check` is `verified` when the quote appears in that text; the comparison looks only at letters and digits and accepts parts separated by `...` in any order, so PDF spacing and list formatting don't cause false misses. `source_check` is `observed` when the cited URL, DOI, or arXiv ID appeared there. The model can't see or set either result.
...
- **Status**: how many report statements the verifier supported, how much evidence there is, and the review reasons. In the PDF this is a title page: the report's title, the objective's opening paragraph, the verifier's figures, the review reasons, and the run's date, job, and cost, followed by a table of contents. The title is the synthesizer's `title` when its report has one, and otherwise the objective's first sentence, without its request phrasing ("I need a detailed report on") and cut before any list of particulars.
- **Summary**: the synthesizer's `executive_summary`, when it wrote one, in a shaded box before the findings.
- **Findings**: the report's answer, with Markdown headings, lists, tables, and links carried over, and each inline `[sN]` citation linked to its source. Layouts models write in plain text are converted first: a line between two rules of `=` or `-`, a line between runs of `=` (`=== SECTION 1 ===`), or a line of capitals standing alone becomes a heading; bullets written at the margin right after a numbered item are nested under it, and the item is set in bold as their heading; and a run of bullets of ` | `-separated cells, under an optional `(Columns: ...)` line, becomes a table.
...
- **Appendices**: the plan, the evidence ledger (each research pass with its claims, evidence, quote and source checks, contradictions, and unresolved questions), and the verification with its follow-ups.

| Format | What you get |
|---|---|
| `pdf` | The `.tex` source compiled by the first engine found: `latexmk`, `pdflatex`, `lualatex`, `xelatex`, or `tectonic` (`--engine` picks one). Shell escape stays off |
| `tex` | Standalone LaTeX that compiles under pdfLaTeX, XeLaTeX, LuaLaTeX, and Tectonic. Model text is escaped; model-written HTML is printed as text |
| `md`, `html` | The same document as Markdown, and as a single page with linked citations |
...
| `research-report RECORD` or `--job-id ID` | Renders a finished run as PDF (through LaTeX), LaTeX, Markdown, HTML, BibTeX, or JSON | No |
