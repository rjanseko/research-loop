...
With `salvage_exhausted_research` turned on, a scout or deep dive that hits a usage limit, or gives up after repeated tool or output errors (`UnexpectedModelBehavior`, for example a fetch tool that kept failing), gets one more call without tools, which summarizes the evidence that call had already gathered. It sees only that call's tool results, not the whole ledger. Unanswered, failed, and repeated tool calls are left out, and the remaining results share a 64,000-character allowance so that later, more targeted fetches aren't crowded out. Quote checks still run against the captured tool text, but the stored salvage prompt keeps only hashes and sizes of the replayed results. The salvage call has 140,000 tokens, room for one validation retry with a long answer. If it fails as well, the question's result has no claims but keeps the loop's search queries and names the pages it read as follow-ups, so gap analysis can send a deep dive to them. A provider error that the model client's own retries did not clear (`ModelAPIError`, such as a 429 or a 5xx) gets no salvage call, which would go to the same provider: the result keeps the loop's queries and pages in the same way, and the job continues with a review reason naming the error. The planner, gap analyst, synthesizer, and verifier still fail the job on a provider error, so an exhausted balance stops it at the next of them.
