...
| `RESEARCH_SCHOLAR_CACHE_MODE` | `live` | Search, fetch, and scholarly cache mode for library runs (`off`, `live`, `record`, `replay`, or `reuse`); see [acquisition.md](acquisition.md#caching) |
...
`scripts/analyze_run.sql` analyses one job from these tables: cost, tokens, and time by role and model, each research task's input tokens request by request, every search and fetch with its real query or URL, repeated calls, which fetched URLs the evidence ledger cites, the verifier's checks by severity, the share of searches and fetches served from the cache, and each research task's time split into model and tool time. Pass the job ID, and write the output under `benchmark_outputs/`, which git ignores, because it holds real search queries and URLs: `mkdir -p benchmark_outputs/analysis && docker compose exec -T postgres psql -U research -d research_loop -v job=<job_id> < scripts/analyze_run.sql > benchmark_outputs/analysis/<job_id>.txt`. Sections 1, 6, 7, and 8 work on any stored job, section 8 only on runs from 2026-09-25 on; sections 2 to 5 need a captured run.

Capture is off by default, and `research-bench` never turns it on, because transcripts hold exactly the benchmark inputs and fetched content that tool events hash. The table comes from migration `004_research_task_messages.sql`, and the cache-hit column on tool events from `005_research_tool_cache_hits.sql`, so run `research-db migrate` after updating; persisting commands refuse to start until both are applied.
