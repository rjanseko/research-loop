...
- **Refusal fallback.** A route's `refusal_fallback` appears in the policy snapshot only when set. Setting it on the presets' planner and synthesizer changed their fingerprints on 25 September 2026. A job whose call was refused and rerun on the fallback shows both tasks, each with its model. Since 25 September 2026 a provider error also reruns the call on the fallback, and the job's notes say so; such a report was written by the fallback model, so compare it with that model's reports, or leave it out, rather than with the route's own. Streaming the synthesis call, added the same day, changes how the reply arrives, not what the model is sent.
- **Model effort.** Since 25 September 2026, every route on `zai:glm-5.3-flash` thinks at `xhigh` (Z.ai's `max`), whatever its preset gave it (`policy.MODEL_EFFORT`). That changed the `glm-heavy` and `value` cheap scouts from `low`, and so those presets' fingerprints, and moves any route an override puts on Flash. Earlier Flash runs, the scout trial's among them, were at `high` or `low`. The same day, `quality`'s planner and synthesizer moved from Opus 5 at `high` to Opus 5.5 at `medium`, and every Opus 5.5 route runs at `medium`. That changed the fingerprints of `quality`, `breadth`, and `glm-heavy`; compare their runs from before it only as a model change.
